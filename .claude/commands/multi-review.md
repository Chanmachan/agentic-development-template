...
builtin `/ultrareview` との棲み分け:
- **このコマンド** = ローカル / 即時 / 無料 / カスタマイズ可能 (反復レビュー、CI 連携、PR 提出前の最終確認向け)
- **`/ultrareview`** = クラウド / 5-10 分 / 課金 / 独立検証込み (merge 直前のゲート向け)
...
| `/ultrareview` (builtin) | merge 直前の最終確認。独立検証で false positive を抑えたいとき (課金) |
