---
description: merge 承認後の「着地」機械作業を一括化。squash merge → tasks.jsonl を done → todo を done/ へ archive → fetch/prune
argument-hint: "<task-id> [PR番号(省略時は tasks.jsonl から解決)]"
allowed-tools: Bash, Read, Edit, Write
---

# Land Task（タスク着地）

ユーザーが **merge を承認した後**の定型作業（merge と後片付け、台帳更新、todo の archive）を 1 コマンドにまとめる。
毎回手で叩いていた手順を機械化して、完了処理のミスと手間を減らす。

## 前提（重要）
- **merge 承認は人間ゲート**。このコマンドは「ユーザーが merge OK と言った後」にのみ実行する。
  コマンド自身が承認の代わりにはならない。承認がまだなら実行しない。
...
7. **報告** — merge commit / done パス / 台帳更新内容を 1 つにまとめて返す。
