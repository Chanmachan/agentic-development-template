...
# their argument so message text is never scanned as flags. `git -C <dir>` and
# leading `cd <dir>` segments (tracked per segment, in order) move the branch
# check into the directory the commit will actually land in — a hook run from
# a main checkout must not false-block a `cd <feature-worktree> && git commit`.
...
def main():
    ...
    curdir = "."  # shell cwd tracked across leading `cd` segments, in order
    ...
    for toks in segments:
        ...
        if toks[0] == "cd":
            cd_args = [t for t in toks[1:] if not t.startswith("-")]
            if cd_args:
                target = cd_args[0]
                cand = target if os.path.isabs(target) \
                    else os.path.join(curdir, target)
