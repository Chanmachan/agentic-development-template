...
4. **builtin `/ultrareview` との棲み分けが不明確**: 課金される `/ultrareview` をいつ使うべきかが文書化されていない。
...
| `/multi-review` (新規) | 6 専門 reviewer 並列。PR レビューの基本形 |
| `/ultrareview` builtin | merge 直前の最終ゲート (独立検証込み、課金) |
...
- `code-review` skill / `code-reviewer` subagent / `/multi-review` / `/ultrareview` の棲み分けが文書化され、迷わず選べる
...
- builtin `/ultrareview` との混同リスク → README と SKILL.md の使い分け表で「自前: 速い・無料・カスタム、ultrareview: 遅い・課金・独立検証」を 1 行で示す
