...
| `/review-cycle` command | multi-review → fix-review を指摘ゼロまで自動収束させるループ (既定 3 周で人に返す)。downstream から import |
| 外部の独立レビュー機能 | merge 直前の最終ゲート (利用中ツールに存在する場合) |

`code-reviewer` の description を「汎用 / 中間解」と明示する形に更新し、`/multi-review` への誘導を含める。`code-review` skill の SKILL.md と `contexts/review.md` にも 5 手段の使い分け表を入れる。
