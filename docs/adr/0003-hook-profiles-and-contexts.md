...
| `standard` (default) | minimal + protect-config + stop-check + suggest-compact + git-guard | 通常開発 |
| `strict` | standard + commit-quality | 本番直前、リリース前ブランチ |
...
case "$PROFILE" in
  ...
  standard) [[ "$HOOK_NAME" =~ ^(post-lint|protect-config|stop-check|suggest-compact|git-guard)$ ]] || exit 0 ;;
  strict) ;; # 全フック実行
esac
```
...
### 4. suggest-compact フックの追加 (standard 以上)
...
- standard プロファイルに git-guard (PreToolUse Bash) を追加。`git commit --no-verify`/`-n`、main への直接 commit (`ALLOW_MAIN_COMMIT=1` で解除)、main を destination とする force-push、および main checkout 中の force-push をブロックする。
- Cursor ハーネスには git-guard の port が未実装 (membership はハーネスごとに差分あり)。
