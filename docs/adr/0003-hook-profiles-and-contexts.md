...
case "$PROFILE" in
  ...
  standard) [[ "$HOOK_NAME" =~ ^(post-lint|protect-config|stop-check|suggest-compact)$ ]] || exit 0 ;;
  strict) ;; # 全フック実行
esac
```
...
- standard プロファイルに git-guard (PreToolUse Bash) を追加。`git commit --no-verify`/`-n`、main への直接 commit (`ALLOW_MAIN_COMMIT=1` で解除)、main への force-push をブロックする。
