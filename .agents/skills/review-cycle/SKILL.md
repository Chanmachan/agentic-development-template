---
name: review-cycle
description: Use after implementation when lint/typecheck/tests are green, to run multi-review then fix-review repeatedly until there are no actionable findings (clean). Caps rounds to avoid runaway, keeps declined findings with reasons, and never loosens quality gates.
---

# Review Cycle（レビュー収束ループ）

`multi-review`(6 観点並列レビュー) と `fix-review`(指摘対応) を**手で交互に回す手間をなくす**ためのスキル。
1 回起動すると「レビュー → 対応 → 再レビュー」を **clean(対応すべき指摘ゼロ) になるまで自動で回し**、
...
2. **1 周目のレビュー** — `multi-review`(または review-* エージェントを並列起動)を対象差分に対して実行し、
...
- review/fix は別文脈のエージェントに委譲してメイン文脈を汚さない(レビューログを長文で貼り込まない)。
